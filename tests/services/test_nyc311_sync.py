...
def patch_settings(
    ...
    settings = SimpleNamespace(
        ...
        nyc_311_initial_lookback_hours=24,
...
async def test_sync_processes_multiple_pages(
    ...
    upsert_many = AsyncMock(side_effect=[2, 1])
    ...
    assert result.window_started_at == (run_started_at - timedelta(hours=24))
    assert result.window_ended_at == run_started_at
    assert result.checkpoint_created_at == run_started_at
    ...
    mark_succeeded.assert_awaited_once_with(
        db,
        "nyc311",
        latest_created_at=run_started_at,
        records_fetched=3,
        records_upserted=3,
    )
...
async def test_sync_uses_existing_checkpoint_and_page_limit(
    ...
    first_record = build_record(
        "2001",
        datetime(2026, 7, 25, 17, 10, tzinfo=UTC),
    )
    second_record = build_record(
        "2002",
        datetime(2026, 7, 25, 17, 20, tzinfo=UTC),
    )

    client = MagicMock(spec=NYC311Client)
    client.fetch_page = AsyncMock(return_value=[first_record, second_record])
...
async def test_sync_records_failure(
    ...
    client = MagicMock(spec=NYC311Client)
    client.fetch_page = AsyncMock(side_effect=RuntimeError("Temporary synchronization failure"))
